...
The following code snippets show how to display a simple map in `deck.gl`, centred on **Vancouver, Canada**, and use `HexagonLayer` to display the [number of trees across the city](https://opendata.vancouver.ca/explore/dataset/public-trees/) in the form of hexagonal bars. The entire source code of the example can be accessed from [here](!).
...
Here is the output of the example above (the sliders have been added and are not provided by `deck.gl`)
```{=html}
<div class='block'></div>
